                            ...
                            <select multiple="multiple" size="30" name="duallistbox_demo2"
                                ...
                                <c:if test="${not empty lstNoAss}">
                                    <c:forEach var="entity1" items="${lstNoAss}"
                                        ...
                                        <option value="${entity1.roleId}">${entity1.name}-${entity1.note}</option>
                                    </c:forEach>
                                </c:if>
                                <c:if test="${not empty lstOfEmp}">
                                    <c:forEach var="entity2" items="${lstOfEmp}"
                                               varStatus="loopCounter">
                                        <option value="${entity2.roleId}" selected="selected">${entity2.name}-${entity2.note}</option>
                                    </c:forEach>
                                </c:if>
                            </select>
                            <script>
                                var demo2 = $('.demo2').bootstrapDualListbox({
                                    nonSelectedListLabel: 'Nhóm quyền chưa dùng',
                                    selectedListLabel: 'Nhóm quyền đã dùng',
                                    preserveSelectionOnMove: 'moved',
                                    moveOnSelect: false,
                                    moveSelectedLabel: "Di chuyển đã chọn",
                                    filterPlaceHolder: 'Nhóm quyền',
                                    infoText: 'Tất cả',
